...
public class ETLMainProcessor {
	...
	if (Files.exists(Paths.get(GlobalValues.FILE_PATH))) {
		...
		// Load VASI Data. First try to connect to VASI Database. If DB is not
		// accessible, then look for a VASI input file
	 ...
	 * updatedRec.setDescription(description); if (description != null &&
	 * description.length() > 255) { System.out.println(
	 * vasiRecord.getSystemID().intValue() + " has a description length of " +
	 * description.length()); } }
	 * 
	 * // Compare Portfolio String portfolio =
	 * compareValues(vasiRecord.getPortfolio(), cmdbRecord.getPortfolio()); if
	 * (portfolio != null) { updateFlag = true; updatedRec.setPortfolio(portfolio);
	 * }
	 * 
	 * // Compare ServiceCategory String serviceCategory =
	 * compareValues(vasiRecord.getServiceCategory(),
	 * cmdbRecord.getServiceCategory()); if (serviceCategory != null) { updateFlag =
	 * true; updatedRec.setServiceCategory(serviceCategory); }
	 * 
	 * // Compare BusinessImpact String businessImpact =
	 * compareValues(vasiRecord.getBusinessImpact(),
	 * cmdbRecord.getBusinessImpact()); if (businessImpact != null) { updateFlag =
	 * true; updatedRec.setBusinessImpact(businessImpact); }
	 * 
	 * // Compare AccreditationBoundary String accreditationBoundary =
	 * compareValues(vasiRecord.getAccreditationBoundary(),
	 * cmdbRecord.getAccreditationBoundary()); if (accreditationBoundary != null) {
	 * updateFlag = true;
	 * updatedRec.setAccreditationBoundary(accreditationBoundary); }
	 * 
	 * // Compare BusinessRisk String businessRisk =
	 * compareValues(vasiRecord.getBusinessRisk(), cmdbRecord.getBusinessRisk()); if
	 * (businessRisk != null) { updateFlag = true;
